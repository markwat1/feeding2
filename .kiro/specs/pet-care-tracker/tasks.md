...
- [x] 6. メンテナンス機能の API 実装
  ...
  - メンテナンス記録 API（GET /api/maintenance, POST /api/maintenance/water-filter, POST /api/maintenance/litter-box, POST /api/maintenance/nail-clipping）
...
- [x] 8.1 摂食状況記録UIの改善
  ...
  - 前回の摂食状況記録セクションのUI変更
  - プルダウン選択と記録ボタンを「食べきった」「残した」の2つのボタンに置き換え
  - ボタンクリックで即座に摂食状況を更新する機能の実装
  - _要件: 3.3, 3.4, 3.5_

- [x] 8.2 餌やり記録画面のコンポーネントテスト
...
- [x] 11.1 メンテナンス記録画面のコンポーネントテスト

  - メンテナンス記録フォームのテスト
  - 履歴表示機能のテスト
  - _要件: 6.1, 6.2, 6.3, 6.5_

- [x] 12. アプリケーション統合とナビゲーション
